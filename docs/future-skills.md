# UI / Components
shadcn/ui - https://ui.shadcn.com/docs ✅ (skill generated)
Radix UI - https://www.radix-ui.com/docs ✅ (skill generated)
Chakra UI - https://chakra-ui.com/docs ⚠️ (anti-scraping detected)
Material UI (MUI) - https://mui.com/material-ui/getting-started/overview/ ✅ (skill generated)
Ant Design - https://ant.design/docs/react/introduce ✅ (skill generated)
Mantine - https://mantine.dev/docs/ ✅ (skill generated, 148 pages)
DaisyUI - https://daisyui.com/docs/ ⚠️ (anti-scraping detected)
Headless UI - https://headlessui.com/react ✅ (skill generated, 27 pages)
Bootstrap - https://getbootstrap.com/docs/ ⚠️ (anti-scraping detected - only 1 page)
Tailwind CSS - https://tailwindcss.com/docs ⚠️ (manual URL list approach needed)

# Design / Storybook / UX
Figma API - https://www.figma.com/developers/api
Storybook - https://storybook.js.org/docs ✅ (skill generated, ~20 pages)
Framer - https://www.framer.com/docs/
Lucidchart API - https://lucid.app/documents/api
Canva API - https://www.canva.dev/docs/

# Frontend Frameworks
React - https://react.dev ✅ (skill generated, 150+ pages)
Next.js - https://nextjs.org/docs ✅ (skill generated, 150+ pages)
Vue.js - https://vuejs.org/guide/introduction.html ✅ (skill generated, 150+ pages)
Nuxt - https://nuxt.com/docs
Svelte - https://svelte.dev/docs
SvelteKit - https://kit.svelte.dev/docs
Angular - https://angular.dev
Remix - https://remix.run/docs
SolidJS - https://www.solidjs.com/docs
Qwik - https://qwik.builder.io/docs/
Astro - https://docs.astro.build

# Backend / Fullstack
Node.js - https://nodejs.org/en/docs
Express - https://expressjs.com/en/starter/installing.html ⚠️ (config created, 1 page - blocked)
NestJS - https://docs.nestjs.com ⚠️ (config created, 1 page - blocked)
Fastify - https://fastify.dev/docs/latest/
Django - https://docs.djangoproject.com/en/stable/ ⏳ (config ready, ~20 pages - generation pending)
Flask - https://flask.palletsprojects.com/en/latest/
FastAPI - https://fastapi.tiangolo.com/ ✅ (skill generated, 150+ pages)
Laravel - https://laravel.com/docs
Ruby on Rails - https://guides.rubyonrails.org
Spring Boot - https://docs.spring.io/spring-boot/docs/current/reference/html/
ASP.NET Core - https://learn.microsoft.com/en-us/aspnet/core/?view=aspnetcore-8.0

# Styling / Animation
Framer Motion - https://www.framer.com/motion/
GSAP - https://gsap.com/docs/
ThreeJS - https://threejs.org/docs/
Emotion - https://emotion.sh/docs/introduction
Styled Components - https://styled-components.com/docs
Vanilla Extract - https://vanilla-extract.style/documentation

# Databases / ORMs
Prisma - https://www.prisma.io/docs ✅ (skill generated, 150+ pages)
Sequelize - https://sequelize.org/docs/ ⚠️ (config created, 1 page - blocked)
TypeORM - https://typeorm.io/
Drizzle ORM - https://orm.drizzle.team/docs/overview ⚠️ (404 error - URL redirect issue)
Mongoose - https://mongoosejs.com/docs/ ⚠️ (anti-scraping detected - only 1 page)
Supabase - https://supabase.com/docs ✅ (skill generated, 150+ pages)
Firebase - https://firebase.google.com/docs
PlanetScale - https://planetscale.com/docs
Neon - https://neon.tech/docs

# API Gateways / Auth
Auth0 - https://auth0.com/docs
Clerk - https://clerk.com/docs
Supabase Auth - https://supabase.com/docs/guides/auth
Firebase Auth - https://firebase.google.com/docs/auth
NextAuth.js - https://next-auth.js.org/getting-started/introduction
Passport.js - https://www.passportjs.org/docs/
Kong - https://docs.konghq.com/
Tyk - https://tyk.io/docs/

# Infrastructure / IaC
Terraform - https://developer.hashicorp.com/terraform/docs ⚠️ (config created, 1 page - blocked)
Pulumi - https://www.pulumi.com/docs/
Ansible - https://docs.ansible.com/
Chef - https://docs.chef.io/
SaltStack - https://docs.saltproject.io/
Serverless Framework - https://www.serverless.com/framework/docs/

# Dev Tools / Platforms
GitHub Actions - https://docs.github.com/en/actions
Docker - https://docs.docker.com ⚠️ (scrape timeout - ~20 pages available)
Kubernetes - https://kubernetes.io/docs/
Vercel - https://vercel.com/docs ✅ (skill generated, ~20 pages)
Netlify - https://docs.netlify.com
Cloudflare - https://developers.cloudflare.com/
AWS - https://docs.aws.amazon.com/
Google Cloud - https://cloud.google.com/docs
Azure - https://learn.microsoft.com/en-us/azure/?product=popular

# Observability / Logging
Grafana - https://grafana.com/docs/
Prometheus - https://prometheus.io/docs/introduction/overview/
OpenTelemetry - https://opentelemetry.io/docs/
Datadog - https://docs.datadoghq.com/
Sentry - https://docs.sentry.io/
New Relic - https://docs.newrelic.com/

# Dev Environments
VS Code API - https://code.visualstudio.com/api
JetBrains Plugin SDK - https://plugins.jetbrains.com/docs/intellij/welcome.html
GitLab - https://docs.gitlab.com/
Bitbucket - https://support.atlassian.com/bitbucket-cloud/docs/
Postman API - https://learning.postman.com/docs/
Insomnia - https://docs.insomnia.rest/

# Languages
Python - https://docs.python.org/3/
JavaScript (MDN) - https://developer.mozilla.org/en-US/docs/Web/JavaScript
TypeScript - https://www.typescriptlang.org/docs/ ✅ (skill generated, ~20 pages)
Go - https://go.dev/doc/
Rust - https://doc.rust-lang.org/
Java - https://docs.oracle.com/en/java/
C# - https://learn.microsoft.com/en-us/dotnet/csharp/
C++ - https://cplusplus.com/doc/
PHP - https://www.php.net/docs.php
Ruby - https://www.ruby-lang.org/en/documentation/
Swift - https://developer.apple.com/documentation/swift

# AI / LLMs / ML Frameworks

## Major Model Providers
OpenAI API - https://platform.openai.com/docs ⚠️ (anti-scraping detected)
Anthropic (Claude) - https://docs.anthropic.com/
OpenRouter - https://openrouter.ai/docs
Mistral - https://docs.mistral.ai/
Cohere - https://docs.cohere.com/
Google Gemini - https://ai.google.dev/
xAI (Grok) - https://x.ai/
Perplexity API - https://docs.perplexity.ai/
Stability AI (Stable Diffusion) - https://platform.stability.ai/docs
Replicate - https://replicate.com/docs
Hugging Face API - https://huggingface.co/docs/api
Together AI - https://docs.together.ai/docs
Ollama - https://github.com/ollama/ollama/blob/main/docs/api.md
LLaMA (Meta) - https://llama.meta.com/docs/
Groq API - https://console.groq.com/docs
Fireworks AI - https://docs.fireworks.ai/
Vercel AI SDK - https://sdk.vercel.ai/docs

## Embeddings / Vector DBs
Pinecone - https://docs.pinecone.io/
Weaviate - https://weaviate.io/developers/weaviate
Qdrant - https://qdrant.tech/documentation/
Milvus - https://milvus.io/docs
Chroma - https://docs.trychroma.com/
FAISS - https://faiss.ai/
PGVector - https://github.com/pgvector/pgvector#readme

## Open Source Model Runtimes / Frameworks
Ollama - https://ollama.com/library
LM Studio - https://lmstudio.ai/docs
vLLM - https://docs.vllm.ai/
Text Generation Inference (TGI) - https://huggingface.co/docs/text-generation-inference/index
FastChat (Vicuna/Chatbot Arena) - https://github.com/lm-sys/FastChat
ExLlama - https://github.com/turboderp/exllama
Llama.cpp - https://github.com/ggerganov/llama.cpp
Whisper (Speech-to-Text) - https://github.com/openai/whisper
Whisper.cpp - https://github.com/ggerganov/whisper.cpp

## Fine-tuning / Training
Axolotl - https://github.com/OpenAccess-AI-Collective/axolotl
Unsloth - https://unsloth.ai/docs
Hugging Face Transformers - https://huggingface.co/docs/transformers/
PEFT (Parameter-Efficient Fine-Tuning) - https://huggingface.co/docs/peft/
DeepSpeed - https://www.deepspeed.ai/docs/
Lightning AI (PyTorch Lightning) - https://lightning.ai/docs
Colossal-AI - https://colossalai.org/docs/
LoRA / QLoRA - https://huggingface.co/docs/transformers/main/en/peft

## AI Orchestration / Agents
LangChain - https://python.langchain.com/docs/ ✅ (skill generated, 150+ pages)
LangGraph - https://langchain-ai.github.io/langgraph/
CrewAI - https://docs.crewai.com/
AutoGen (Microsoft) - https://microsoft.github.io/autogen/
Semantic Kernel - https://learn.microsoft.com/en-us/semantic-kernel/
OpenDevin - https://docs.opendevin.org/
AgentVerse - https://docs.agentverse.ai/
SuperAGI - https://docs.superagi.com/
CAMEL-AI - https://github.com/camel-ai/camel

# AI Coding Agents / AI Developer Tools
OpenCode - https://opencode.ai/docs/
GitHub Copilot Coding Agent - https://docs.github.com/en/copilot/concepts/agents/coding-agent/about-coding-agent
GitHub Copilot Agent How-To - https://docs.github.com/en/copilot/how-tos/use-copilot-agents/coding-agent
Roo Code - https://docs.roocode.com/
Agent.ai - https://docs.agent.ai/
AWS Bedrock Agents - https://docs.aws.amazon.com/bedrock/latest/userguide/agents.html
Stately Agents - https://stately.ai/docs/agents
v0 (by Vercel) - https://v0.dev/docs
Bolt.new - https://support.bolt.new/
Lovable - https://docs.lovable.dev/
Codebolt - https://docs.codebolt.ai/docs/developer/agents/agentIntroduction
Cody (Sourcegraph) - https://docs.sourcegraph.com/cody
Continue.dev - https://docs.continue.dev/
Sweep.dev - https://docs.sweep.dev/
Plandex - https://docs.plandex.ai/
Supermaven - https://supermaven.com/docs
Devin (Cognition) - https://www.cognition.ai/
Smol Developer - https://github.com/smol-ai/developer
Codeium - https://codeium.com/docs
Replit Agent - https://docs.replit.com/ai
Cursor - https://cursor.sh/docs
Blackbox AI - https://www.useblackbox.io/docs
Tabnine - https://www.tabnine.com/docs
Warp AI - https://docs.warp.dev/features/ai
Pieces.app (AI Dev Assistant) - https://docs.pieces.app/
Claude Workbench - https://docs.anthropic.com/en/docs/building-with-claude/workbench
aider.chat - https://aider.chat/docs/
SmolAgent - https://github.com/huggingface/smolagents
SuperAGI - https://docs.superagi.com/
OpenDevin - https://docs.opendevin.org/
CrewAI - https://docs.crewai.com/
Dust.ai - https://docs.dust.tt/
Griptape - https://docs.griptape.ai/
Continue Plugin (VS Code) - https://marketplace.visualstudio.com/items?itemName=Continue.continue

## RAG / Knowledge Systems
LlamaIndex (formerly GPT Index) - https://docs.llamaindex.ai/
Haystack - https://docs.haystack.deepset.ai/docs/intro
Ragas - https://docs.ragas.io/en/latest/
Marqo - https://docs.marqo.ai/
Raptor - https://github.com/parlance-labs/raptor#readme

## Evaluation / Monitoring / Tooling
Weights & Biases (W&B) - https://docs.wandb.ai/
TruLens - https://www.trulens.org/docs/
Evals (OpenAI) - https://github.com/openai/evals
LangFuse - https://docs.langfuse.com/
Helicone - https://docs.helicone.ai/
PromptLayer - https://www.promptlayer.com/docs
Braintrust - https://docs.braintrust.dev/
HoneyHive - https://docs.honeyhive.ai/
Arize Phoenix - https://docs.arize.com/phoenix/
WhyLabs - https://whylabs.ai/docs

## Speech / Vision / Multimodal
Whisper - https://github.com/openai/whisper
Whisper.cpp - https://github.com/ggerganov/whisper.cpp
OpenAI GPT-4o - https://platform.openai.com/docs/guides/gpt-4o
CLIP - https://github.com/openai/CLIP
BLIP - https://huggingface.co/docs/transformers/model_doc/blip
SAM (Segment Anything) - https://segment-anything.com/
Gemini Multimodal - https://ai.google.dev/gemini-api/docs

## Prompting / Templates
Promptify - https://github.com/promptslab/Promptify
PromptLayer - https://www.promptlayer.com/docs
Flowise - https://docs.flowiseai.com/
Dify AI - https://docs.dify.ai/
Griptape - https://docs.griptape.ai/
Dust - https://docs.dust.tt/
Vercel AI SDK - https://sdk.vercel.ai/docs
Chatbot UI - https://github.com/mckaywrigley/chatbot-ui
OpenDevin - https://docs.opendevin.org/

# AI / ML / Data
PyTorch - https://pytorch.org/docs/stable/
TensorFlow - https://www.tensorflow.org/api_docs
Pandas - https://pandas.pydata.org/docs/
NumPy - https://numpy.org/doc/
scikit-learn - https://scikit-learn.org/stable/user_guide.html
FastAPI - https://fastapi.tiangolo.com/
Streamlit - https://docs.streamlit.io/

# Testing
Jest - https://jestjs.io/docs/getting-started ⚠️ (anti-scraping detected - only 1 page)
Vitest - https://vitest.dev/guide/ ⚠️ (insufficient pages - only 2 pages)
Playwright - https://playwright.dev/docs/intro ⚠️ (anti-scraping detected - only 1 page)
Cypress - https://docs.cypress.io/
Testing Library - https://testing-library.com/docs/
Pytest - https://docs.pytest.org/en/stable/
Mocha - https://mochajs.org/
Jasmine - https://jasmine.github.io/pages/docs_home.html

# Package Managers / Build Tools
npm - https://docs.npmjs.com/
pnpm - https://pnpm.io/motivation
Yarn - https://yarnpkg.com/getting-started
Vite - https://vitejs.dev/guide/
Webpack - https://webpack.js.org/concepts/
Rollup - https://rollupjs.org/
esbuild - https://esbuild.github.io/
Bun - https://bun.sh/docs

# Misc / Utilities
Zod - https://zod.dev
Axios - https://axios-http.com/docs/intro
Day.js - https://day.js.org/docs/en/installation/installation
Lodash - https://lodash.com/docs/
Moment.js - https://momentjs.com/docs/
RxJS - https://rxjs.dev/guide/overview
tRPC - https://trpc.io/docs
OpenAPI - https://swagger.io/docs/specification/about/
